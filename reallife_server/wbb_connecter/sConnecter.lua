...
function Cwbbc:message(sMessage)
	outputDebugString(("[%s@%s]: %s"):format(self.sUser, self.sDBName, sMessage))
end
...
function registerForumWBB (player, pw, email)
if forumsync == 1 then
 ...
 local id =   tonumber (getUserID(pname))
  dbExec ( handler, "INSERT INTO forum_accounts ( UID,ForumID ) VALUES (?,?)", playerUID[pname],id )
 outputChatBox ( "Dein Account mit dem Passwort "..pw.." und der E-Mail "..email.." wurde eingerichtet!", player, 0,139,0 )
 end
end
addEvent ( "registerForumWBB", true )
addEventHandler ( "registerForumWBB", getRootElement(), registerForumWBB )


function loadForumPermissions(client)

local player = getPlayerName(client)
local UserID =   tonumber (getUserID(player))
if vioGetElementData ( client, "adminlvl" ) < 0 then
	addUserToGroup(UserID, adminForumRank[vioGetElementData ( player, "adminlvl" )])
	addUserToGroup(UserID, teamid)
else
	for index, i in pairs(adminForumRank) do
        removeUserFromGroup(UserID, i)
    end
	removeUserFromGroup(UserID, teamid)
if vioGetElementData ( client, "premium") == true then
	addUserToGroup(UserID, VIPGroupe)
else
	removeUserFromGroup(UserID, VIPGroupe)
end
end
end