...
addEventHandler ( "tutorialended", root, function ( )
	...
	triggerClientEvent ( client, "newForumAccount", client )
...
function saveLevelSystem ( player )
	local pname = getPlayerName ( player )
	dbExec ( handler,"UPDATE ?? SET ??=?, ??=?, ??=?, ??=? WHERE ??=?", "levelsystem", "MainXP", vioGetElementData ( player, "MainXP" ), "MainLevel", vioGetElementData ( player, "MainLevel" ), "GWXP", vioGetElementData ( player, "GWXP" ), "GWLevel", vioGetElementData ( player, "GWLevel" ), "UID", playerUID[pname])
end
	...
	if result and result[1] then
		...
		if tonumber ( dsatz["ForumID"] ) then
			if tonumber ( dsatz["ForumID"] ) == 0 then
				exports["dxSystem"]:openForum(player)
			else
